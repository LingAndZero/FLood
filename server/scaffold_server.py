...
class SCAFFOLDServer:
    ...
    # For aggregate
    def aggregate(self, local_models, local_n_train, local_variates):
        ...
        num = len(local_variates)
        global_variate_next = {k: torch.zeros_like(v, device="cpu") for k, v in self.global_variate.items()}

        for lv in local_variates:
            for k, v in lv.items():
                global_variate_next[k].add_(v.detach().cpu(), alpha=1.0 / num)

        self.global_variate = global_variate_next
