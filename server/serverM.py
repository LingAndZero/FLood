...
class ServerM:
    ...
    def __init__(self, args, dataset_test, checkpoint_logdir):
        ...
        self.v = {name: None for name in self.model_g.state_dict().keys()}
    ...
    # For aggregate
    def aggregate(self, local_models, local_n_train, _):
        ...
        for param_name in global_state:
            ...
            if self.v[param_name] is None:
                self.v[param_name] = weighted_sum.clone()
            else:
                self.v[param_name].mul_(self.args.beta)
                self.v[param_name].add_(weighted_sum)
            
            w_glob[param_name] = (self.v[param_name] + flat_global).view(param_shape)
